...
public:
    explicit Macroparameters1dOutput(std::filesystem::path folderName)
        : folderPath(std::move(folderName))
    {
        std::error_code errorCode;
        if (!std::filesystem::create_directories(folderPath, errorCode) && errorCode) {
            std::cerr << "folder \"" << folderPath.string() << "\" wasn't created: " << errorCode.message() << '\n';
        }

        t_output.open(folderPath / "t_levels.txt");
        x_1_output.open(folderPath / "x_1_levels.txt");

        n_output.open(folderPath / "n.txt");
        u_1_output.open(folderPath / "u_1.txt");
        T_output.open(folderPath / "T.txt");
        q_1_output.open(folderPath / "q_1.txt");
    }
    ...
    [[nodiscard]] std::string getFolderName() const { return folderPath.string(); }
};
...
public:
    explicit Full1dStateOutput(const std::filesystem::path& folderName) : mpOutput(folderName)
    {
    ...
    */
    ...
    [[nodiscard]] std::string getFolderName() const { return mpOutput.getFolderName(); }
};
