...
namespace full_boltzmann_1d3v
{
    ...
    struct HardSphereCollisionModel
    {
        ...
        {
            directions = {
                {T(1), T(0), T(0)}, {T(-1), T(0), T(0)}, {T(0), T(1), T(0)}, {T(0), T(-1), T(0)}, {T(0), T(0), T(1)}, {T(0), T(0), T(-1)}};
    ...
    {
        std::filesystem::create_directories(folder_path);
        const MacroState1D3V<T> macro = computeMacroState(state, grid);
        ...
        const std::array<std::pair<const char *, int>, 3> slices = {{{"distribution_left.txt", 0},
...
#pragma omp parallel for
        for (int x_i = 0; x_i < state.n_x; ++x_i)
        {
            ...
            const T temperature = std::max(T(2) * (energy / safe_density - bulk_energy) / T(3), T(1e-8));
        ...
        bool lower_wall)
    {
        if (wall.enabled && !wall.incoming_maxwellian.empty())
        {
            ...
            for (int k = 0; k < grid.size(); ++k)
            {
                ...
                if (lower_wall)
                {
                    if (normal_velocity < T(0))
                    {
                        outgoing_flux += -normal_velocity * state.at(boundary_cell, k);
                    }
                    else if (normal_velocity > T(0))
                    {
                        incoming_wall_flux += normal_velocity * wall.incoming_maxwellian[k];
                    }
                }
                else
                {
            ...
            return scale * wall.incoming_maxwellian[static_cast<std::size_t>(v_i)];
        }
...
#pragma omp parallel for
        for (int x_i = 0; x_i < state.n_x; ++x_i)
        {
            ...
            for (int v_i = 0; v_i < state.n_v; ++v_i)
            {
                ...
                if (x_i == 0 && normal_velocity > T(0))
                {
                    left_neighbor = boundaryIncomingValue(data.velocity_grid, data.left_wall, state, 0, v_i, true);
                }
                if (x_i == state.n_x - 1 && normal_velocity < T(0))
                {
                    right_neighbor = boundaryIncomingValue(data.velocity_grid, data.right_wall, state, state.n_x - 1, v_i, false);
                }

                const T left_state = (normal_velocity >= T(0)) ? left_neighbor : state.at(x_i, v_i);
                const T right_state = (normal_velocity >= T(0)) ? state.at(x_i, v_i) : right_neighbor;

                const T left_flux = normal_velocity * left_state;
                const T right_flux = normal_velocity * right_state;

                T value_rhs = -(right_flux - left_flux) / dx + cell_rhs[v_i];

                const auto [ix, iy, iz] = data.velocity_grid.unflattenIndex(v_i);
                if (std::abs(data.body_force.x) > T(0))
                {
                    const int prev_ix = (ix == 0) ? 0 : ix - 1;
                    const int next_ix = (ix == data.velocity_grid.n_v - 1) ? data.velocity_grid.n_v - 1 : ix + 1;
                    const int prev_v = data.velocity_grid.flattenIndex(prev_ix, iy, iz);
                    const int next_v = data.velocity_grid.flattenIndex(next_ix, iy, iz);

                    const T force_term = (data.body_force.x > T(0))
                                             ? -data.body_force.x * (state.at(x_i, v_i) - state.at(x_i, prev_v)) / data.velocity_grid.dv
                                             : -data.body_force.x * (state.at(x_i, next_v) - state.at(x_i, v_i)) / data.velocity_grid.dv;
